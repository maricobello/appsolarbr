...
def check_password():
    ...
    def password_entered():
        """Verifica se uma senha inserida pelo usuário está correta."""
...
def get_lat_lng(address):
  """
     Obtenha a latitude e longitude de um determinado endereço usando a API Google Geocoding.

     Parâmetros:
     - endereço: O endereço para geocodificar.
...
def plot_roof_segments(roof_segments):
    ...
    ax.set_title('Segmentos de telhado com orientação e inclinação')
...
def plot_3d_roof_segments(roof_segments):
    if not roof_segments:
        st.write("Não há dados disponíveis sobre segmentos de telhado.")
        return
...
def solar_calculator(data):
    ...
    if 'electricity_price' not in st.session_state:
        st.session_state.electricity_price = 0.45
...
def main():
    st.title('Insights do painel solar')
    ...
    if 'data' in st.session_state:
        ...
        with st.expander("Clique para desdobrar a resposta completa da solarAPI", expanded=False):
            st.write(data)
        ...
        if st.button('Get Data Layers'):
            ...
            st.subheader('Modelo Digital de Superfície')
            display_all_geotiff_bands(data_layers['dsmUrl'], GOOGLE_SOLAR_KEY, 'Modelo Digital de Superfície')

            st.subheader('Camada Composta RGB')
            display_all_geotiff_bands(data_layers['rgbUrl'], GOOGLE_SOLAR_KEY, 'Camada Composta RGB')

            st.subheader('Máscara de construção')
            display_all_geotiff_bands(data_layers['maskUrl'], GOOGLE_SOLAR_KEY, 'Máscara de construção')
            ...
            with st.expander("Clique para desdobrar a resposta completa do dataLayer", expanded=False):
