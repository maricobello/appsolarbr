...
def check_password():
    ...
    if "password_correct" not in st.session_state:
        # Primeira execução, exibir entrada para a senha.
        st.text_input(
...
def get_lat_lng(address):
    """
    Obtém a latitude e longitude de um endereço usando a API de Geocodificação do Google.
...
@st.cache_data
def get_google_maps_image(lat, lon, zoom=19, size="600x600", maptype="satélite", api_key=GOOGLE_SOLAR_KEY):
...
# Função para exibir uma banda específica de um arquivo GeoTIFF a partir de uma URL com anotação
def display_monthly_flux(data_layers, api_key):
    # Para o fluxo mensal, você precisa lidar com várias bandas, pois ele contém dados para cada mês
    ...
    if response.status_code == 200:
        with MemoryFile(response.content) as memfile:
            with memfile.open() as dataset:
                for i in range(1, dataset.count + 1):  # Percorrer cada banda
                    ...
                    band = dataset.read(i)
                    im = ax.imshow(band) #cmap='viridis')  # Armazenar o objeto mapeável em im
                    #plt.colorbar(im, ax=ax)  # Passar o objeto mapeável para o colorbar
        ...
        print(f"Falha ao buscar os dados. Código de status: {response.status_code}")
        # Função para exibir todas as bandas de um arquivo GeoTIFF a partir de uma URL com anotação
    ...
    if response.status_code == 200:
        with MemoryFile(response.content) as memfile:
            with memfile.open() as dataset:
                ...
                im = ax.imshow(band)  # Armazenar o objeto mapeável em im, cmap='viridis' se dataset.count == 1 else None
                #plt.colorbar(im, ax=ax)  # Passar o objeto mapeável para o colorbar
                ax.set_title(title)
                st.pyplot(fig)  # Passar a figura do matplotlib para st.pyplot()
                plt.close(fig)  # Fechar a figura

# Função para obter camadas de dados
@st.cache_data
...
def plot_roof_segments(roof_segments):
    ...
    for segment in roof_segments:
        ...
        # Criar um retângulo representando o segmento do telhado
        rect = patches.Rectangle((sw['longitude'], sw['latitude']), width, height,
                                 linewidth=1, edgecolor='r', facecolor='none')
        ax.add_patch(rect)

        # Usar azimute para representar a orientação do segmento do telhado
        azimuth = segment['azimuthDegrees']
        pitch = segment['pitchDegrees']

        # Calcular o ponto final para a linha que indica a orientação
        # Aqui, estamos usando um método simples para calcular o ponto final; você pode precisar ajustar isso
        end_lon = center['longitude'] + np.cos(np.radians(azimuth)) * width * 0.5
        end_lat = center['latitude'] + np.sin(np.radians(azimuth)) * height * 0.5

        # Desenhar uma linha que indica a orientação do segmento do telhado
    ...
    ax.set_title('Segmentos do Telhado com Orientação e Inclinação')
...
def plot_3d_roof_segments(roof_segments):
    if not roof_segments:
        st.write("Nenhum dado de segmentos de telhado disponível.")
        return
    ...
    for segment in roof_segments:
        ...
        # Depuração: Imprimir inclinação e azimute
        #st.write(f"Inclinação: {pitch} radianos, Azimute: {azimuth} radianos")

        # Calcular a diferença de altura com base na inclinação e no azimute
    ...
    # Definir limites dos eixos e rótulos
    ...
    ax.set_title('Visualização 3D de Segmentos de Telhado')
...
def solar_calculator(data):
    ...
    # Extrair número mínimo e máximo de painéis dos resultados da API
    ...
    # st.session_state.num_panels = st.number_input('Número de Painéis Solares', value=st.session_state.num_panels)
    # st.session_state.kw_per_panel = st.number_input('kW por Painel Solar', value=st.session_state.kw_per_panel)
    # st.session_state.electricity_price = st.number_input('Preço da Eletricidade (por kWh)', value=st.session_state.electricity_price)
    # st.session_state.panel_area_m2 = st.number_input('Área por Painel Solar (em m^2)', value=st.session_state.panel_area_m2)

    # Cálculos
    # Entradas
    panels = st.slider('Número de Painéis Solares', min_value=min_panels, max_value=max_panels, value=max_panels)
    user_wattage = st.number_input('Potência do Painel Solar', value=430.0, step=10.0)  # O usuário pode ajustar a potência
    ...
    energia_ajustada = energia_api * (user_wattage / 250.0)  # Ajustar com base na potência especificada pelo usuário
    economia_anual = energia_ajustada * electricity_price

    # Exibir
    st.write(f"Geração Anual de Energia Estimada: {energia_ajustada:.2f} kWh")
    st.write(f"Economia Anual Estimada: €{economia_anual:.2f}")
...
def main():
    ...
    # Verifique se o endereço está na session_state
    if 'address' not in st.session_state:
        st.session_state.address = ''

    # Verifique se os dados já estão na session_state e se o endereço foi alterado
    ...
    if 'data' in st.session_state:
        ...
        # Adicione uma observação usando st.markdown
        ...
        st.write(f"Número Máximo de Painéis na Matriz: {data['solarPotential']['maxArrayPanelsCount']}")
        st.write(f"Área Máxima de Painéis Solares (m^2): {data['solarPotential']['maxArrayAreaMeters2']}")
        ...
        st.write(f"Máximo de Horas de Sol Anuais: {data['solarPotential']['maxSunshineHoursPerYear']}")

        if 'roofSegmentStats' in data['solarPotential']:
            st.subheader('Visualização 2D de Segmentos de Telhado')
            plot_roof_segments(data['solarPotential']['roofSegmentStats'])

            st.subheader('Visualização 3D de Segmentos de Telhado')
